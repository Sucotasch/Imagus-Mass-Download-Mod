...
- **Session-scoped memory ≠ queue state (D-10, 2026-09-12):** `sessionDownloadedKeys` (`chrome.storage.session` key `mdSessionDownloads`, debounce 2 s, FIFO cap 4000) remembers what THIS browser session already downloaded; `mdDedupSkipReason()` is the single decision point. `resetMassDownloadSession()` deliberately does NOT clear it (that is the point — a lazy-load re-scan must not re-fetch and litter `name (1).jpg`); `handleClearAll()` and `onInstalled` do clear it. The skip is always a visible row (`Already downloaded (this session)`), a preserved `completed` row is never downgraded to `skipped`, and explicit Retry bypasses dedup.
