...
- **The SINGLE save (hover toolbar / hotkey) uses the same offscreen tier, deliberately OUTSIDE the session (SAVE-1, 2026-09-21).** Chrome refuses an extension-initiated download of a registry host and the `alterDownload` fallback asks the PAGE for the bytes — a cross-origin `i.pximg.net` response sends no CORS headers, so that fetch dies and the user sees the alert `Download failed: Failed to fetch` with no file (the open item of `Docs/PLAN_96_FOLLOWUP_2026-09-13.md` §D2). `mdSaveViaOffscreen(msg)` (`mass-download/service-core.js`) is called from the `downloads.onChanged` interrupt branch of `background/service.js`, fetches with `mdOffscreenFetchBounded` and hands the object URL to a fresh `download()` (filename kept from the ORIGINAL CDN url — a `blob:` url names nothing). Gates, each one a reason to stay out: `mdOffscreenSupported()` (**Chrome only** — the FF tree's `downloads` API carries the Referer natively, so `mdSaveViaOffscreen` does not exist there at all), `mdDnrRequestFor` (the 4 registry hosts), a LIVE rule (`mdDnrRuleActiveFor`, ensured first), one attempt per item (`_offscreenDone`). Every refusal or failure falls back to the old path unchanged. It writes NO progress row, NO counter and NO queue — a single save is not a scan. `cleanup()` runs BEFORE the async attempt (a downloadId gets ONE verdict: `state` and `error` arrive as separate deltas, and a second one would fetch twice) and revokes via `_objectUrlScope === 'offscreen'`. Live proof, Edge 153 on a scratch profile, read out of `chrome.downloads.search`: `blob:chrome-extension://…` → `md-probe-p0.jpg`, `state: complete`, `image/jpeg`, 543078 B — the real pixiv original that used to end in the alert.
...
- **Restore-dedup has TWO identities, and both are per ITEM, not per object (FIX-DUP-3/4, 2026-09-21).** “Came back from a snapshot” is keyed by `mdRestoredKeys` — `fileKey()` of the task's URL *and* of every candidate in its group — because the filter phase rebuilds the task object whenever it re-routes an item (host pinned to browser download, offscreen tier, next-candidate advance) and none of those clones carries `_restored`: measured on the 11:38 run, where **all 43 cross-generation ` (1)` pairs** went down that cloned path (rows even lost `restored: yes`). And the Chrome-history window is bounded by the run's EARLIEST start (`mdRunOrigin`, carried in the snapshot as `runOrigin`, combined by the pure `mdNextRunOrigin` as a minimum), never by `sessionStartTime`: `mdApplySnapshot` re-keys that to the recovering worker and each snapshot stored the re-keyed value, so the window slid forward one generation per restart (28 of those 43 pairs predated it). `resetMassDownloadSession()` clears both — a new scan is a new run, and a deliberate re-download of a page still works. Locks live in `md-unit-smoke` (executed) + `mutation-check`.
- **Testing the extension itself: reload it, do not just relaunch.** An unpacked extension's `importScripts()` sub-scripts come from the script cache until the extension is reloaded or its version changes, so a probe run after editing `mass-download/*.js` silently executes the OLD code (`ReferenceError: mdIsRestoredTask is not defined`, measured 2026-09-21) and reports the old version. Use `ext-dev-loop` with `--reload` (or bump the version), and read the log's `Version:` line before believing any result — the ambiguity of the 11:10 log came from exactly this.
...
- **A restored task is checked against Chrome's download history before it downloads again (DUP-1, 2026-09-21).** A worker can die after Chrome wrote the file but before the debounced snapshot recorded the completion, so the row comes back non-terminal and is re-queued — the 2026-09-20 log left `b8e371b4…png` and `b8e371b4… (1).png` (same bytes, two generations). `mdAdoptIfAlreadyDownloaded()` (service-core.js) adopts an already-written file through `updateDownloadProgress` instead of creating a second copy — since FIX-DUP-2 (below) it answers from a session-scoped index rather than a per-URL search. **Scope: `task._restored` only** (set when a snapshot requeues) — a fresh scan must stay able to re-download a page on request, and a deleted file (`exists === false`) must be downloaded again. `_historyPending` parks the task for exactly one answer because the drain loop is synchronous. **The question must be MARKED asked** — `task._historyChecked = true;` in the `'check'` branch of `processDownloadQueue`, i.e. at ask time: 2026-09-21 the flag was read there and written nowhere, so every restored item was parked by its own answer, re-picked, asked again, forever. The live 09-27-34 log shows the shape: `queued=4`, `filtering=0 downloading=0`, `!! STALLED`, the panel frozen on 'Scanning', and a restored pixiv gallery save that never began. The three states are one pure function (`mdHistoryGuard`: `'check'` / `'park'` / `'proceed'`) so `tools/md-unit-smoke.mjs` can EXECUTE the sequence and prove it terminates (a source that asks twice cannot leave the loop), and `mdAdoptIfAlreadyDownloaded` resumes the queue in a `finally` — a throw inside the adopt path must not strand a parked task either.

- **The log must NAME a duplicate file, not just count rows (LOGFMT-1, 2026-09-21).** A run left 297 files on disk with **36 byte-identical `имя (1).png` pairs** and **29 pictures stored twice (sample + original)** while the log showed ONE row per URL — and the reason the evidence was invisible is structural: (a) the item table is capped at `da.maxProgressRecords`, so the 10:34 run shipped 100 of its 296 completed rows and the duplicates' rows were exactly the ones dropped, and (b) nothing recorded the name **Chrome wrote**. Chrome never overwrites — it appends ` (1)` — so `requestedName` (ours) vs `recordedName` (Chrome's, from `results[0].filename`) IS the duplicate fingerprint, and `browserId` ties the row to Chrome's own history entry. Save Log now ships `outcomes` (`mdOutcomesForLog()` — the **uncapped** `mdOutcomeByUrl` ledger, printed as `status url`) plus the per-row `recordedName`/`uniqName`/`browserId`/`restored`/`historyAdopted`, and `formatLog` prints a **DUPLICATE FILE NAMES** block (rows grouped by written name) and the Recovered line's `N adopted from Chrome's download history`. Locks `LOGFMT-1` in `tools/md-unit-smoke.mjs` (both trees) execute `mdBasename` and assert both ends of the fingerprint survive a refactor. Lesson: when a symptom is only visible on disk, the log — not the download folder — must carry the field that proves it.

- **An offscreen delivery that was not ANSWERED is not a refusal (SAVE-2, 2026-09-21).** `chrome.runtime.sendMessage` fails two different ways and `mdOffscreenSend` used to handle only one: **no receiver** → it rejects (document still loading, or — the frequent case — it self-closed: `offscreen/offscreen.js` closes itself after `IDLE_CLOSE_MS` 30 s while the worker's cached `mdOffscreenSetup` stays resolved, and a worker easily outlives 30 s); **a receiver that does not answer** → it RESOLVES WITH `undefined`, which is what an open **progress tab** causes: the tab registers `chrome.runtime.onMessage` and ignores commands it does not know, so a message the closed document never saw came back as a silent `undefined`, the tier was read as "refused", and the save fell back to the page-context fetch that a Referer-gated CDN can never satisfy — the live "single saves on pixiv work, then fail with *Failed to fetch*, on the same page and the same picture". A delivery now counts only when an ANSWER arrives; otherwise the stale cache is dropped, `mdOffscreenEnsure()` runs again, and the message is retried on a bounded ladder (`MD_OFFSCREEN_SEND_MS` 150/300/600 ms). `revive === false` keeps the object-URL revoke fire-and-forget (never resurrect a document to revoke a URL that died with it). Locks `SAVE-2` execute all four cases (silent answer retried + exactly one recreate, rejection retried, revoke never recreates, ladder bounded).

- **The restore-dedup question is asked about the RUN, not about one URL (FIX-DUP-2, 2026-09-21).** Two live runs bracket the answer: 10:34 = FIVE worker generations (four `abrupt` deaths) → 36 byte-identical `имя (1)` pairs + 29 pictures stored twice (the full-size file written by an older generation, the sample by the restored one); 11:10 = ONE generation, no restart → 191 files, ZERO duplicates (folder `02`). Duplicates are therefore a **recovery** phenomenon, and the old per-URL `chrome.downloads.search({url})` could not see them: (a) the bytes may have been handed to Chrome as a **`blob:` URL** (offscreen tier, page/referer fetch) and Chrome records the blob URL — the http URL is nowhere in its history; (b) a restored task re-runs its candidate chain **from the top**, so it can land on a different candidate of the SAME item (the 29 sample/original pairs). Now: one session-scoped query (`mdWrittenIndexRefresh`, `startedAfter` the RUN's start — `mdSessionOriginMs()` prefers the snapshot's `sessionStart`, because `mdApplySnapshot` re-keys `sessionStartTime` to the recovering worker) builds an index keyed by URL **and by the file name Chrome wrote**; `mdFindAlreadyWritten(task, index)` (pure, executed by the smoke harness) matches (a) the task's URL, (b) any candidate of its group, (c) the target name — and `mdTargetName` refuses to treat `full`/`index.php`-style shared names as identity. A matching download that is still `in_progress` **parks** the task and re-asks (bounded by `MD_HISTORY_WAIT_TRIES` × `MD_HISTORY_WAIT_MS`), because racing Chrome's own write IS the ` (1)` copy; a deleted file (`exists === false`) still downloads. Scope stays `_restored` — a fresh scan must stay able to re-download a page on request, and `resetMassDownloadSession` drops the index so it cannot adopt its own previous files. Every `complete` feeds the index at once (`mdWrittenIndexNote`).

- **A release commit must be an ancestor of every fix it claims (2026-09-21).** Tag `v2026.9.6.1` is commit `1d9a769`, and `a00e4ac` — committed 35 seconds earlier — is **not** an ancestor, so the released archive shipped the very bug that commit fixes (the worker deaf to its own page while it boots: the page finds ~180 items, the worker takes 8, the panel announces completion). The tag's own tree proves it: `git show v2026.9.6.1:src-mv3-overlay/background/service.js` still registers `onUserScriptMessage` inside the awaited `registerContentScripts()`. Before cutting a release: `git merge-base --is-ancestor <fix-commit> <release-commit>` for every fix, and re-check that the zips were built from that tree. Also remember the archives do **not** track the working tree — a test run reports the LAST RELEASED version, which is why the manifests were bumped to `2026.9.21.1` this day.
