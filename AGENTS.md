...
- **Dedup is by FILE IDENTITY KEY (stage 4a)**: `fileKey()` in SW == `_normalizeUrlKey()` in content (strip HD `#`, resolve `//…` → `https://…`, drop query, collapse `//` in path, `.jpeg` → `.jpg`). Owners: `PVI.downloadAllUniqueUrls` (content), `globalProcessedUrls` (SW — single add-point in `processFilterQueue`; explicit retries exempt). Separately, `candidateKey()` (keeps extension + query) dedups alternatives INSIDE a group so fallback chains never lose a real candidate. The content copy is inline (I1), so the two implementations must stay in sync — `tools/md-unit-smoke.mjs` asserts their equivalence. History: the 2026-07-25 half-normalized attempt was rolled back in v2026.7.25.6; the stage-4a two-key contract supersedes it.
