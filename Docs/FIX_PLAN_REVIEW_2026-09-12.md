...
#### D-9 (сделано, информационно)

Отсюда и решение: вместо таймера, который закрывал бы сессию (риск — отмена здорового скана; в
скрытой/замороженной вкладке Chrome тормозит таймеры, и пауза в несколько минут может быть
нормальной, а задачи, пришедшие после `scanInProgress=false`, помечаются `canceled`), добавлен
**детектор молчания страницы**:

- `mdContentSeenAt` обновляется только сообщениями, которые может прислать ТОЛЬКО сканируемая
  страница (`MD_PAGE_MSG_CMDS` в `handleMessage`; опрос вкладки прогресса — раз в 5 с — сознательно
  не считается, иначе он маскировал бы мёртвую страницу);
- раз в 30 с (alarm keep-alive) проверяется: сессия открыта, `contentScanDone` нет, **в полёте ничего
  нет** и страница молчит дольше `max(3 мин, 3 × resolutionTimeout)` → один раз в окно молчания
  в консоль SW и в строку статуса вкладки выводится «No word from the page for N min and nothing in
  flight — the scan looks interrupted (page closed, navigated away or frozen?). The session and its
  snapshot are kept»;
- `contentSeenAt` живёт в снапшоте → после рестарта воркера страница, замолчавшая до смерти, не
  получает новый бюджет молчания;
- в Save Log добавляется строка `Page silent: Ns while nothing was in flight`.

Свойства, залоканные тестами: детектор **ничего не отменяет** (не переводит строки в `failed`, не
закрывает сессию), **молчит при любой работе в полёте** (активный фильтр/загрузка/ретрай) и **масштабируется** с
`resolutionTimeout` — то есть не может сработать внутри легитимного ожидания. Если измерение на живой
медленной галерее покажет, что пауз такого размера не бывает, детектор можно повысить до действия
(закрыть сессию / перевести строки в `failed`) — но только после измерения, а не по догадке.
