...
- остаётся промежуточный случай — пользователь переключился в другое окно (страница скрыта, но не
  мертва): Chrome тормозит таймеры до ~1 раза в минуту, сообщения при этом доходят. Порог 3 минуты
  (и запас ×3 к `resolutionTimeout`) покрывает и это без ложных срабатываний;
...
**D-11 (открыто, новое):** `_stopKeepAwake` выключается, когда скан завершает **контент-скрипт**, а SW
после этого ещё может нуждаться в странице (referer-ретраи на групповых элементах в фазе
фильтра/загрузки). Пока вкладка просто скрыта, это безопасно (сообщения доставляются, тормозятся
только таймеры); при полной заморозке (скрытая >5 мин, аудио уже выключено) fetch не выполнится и через
30 с сработает watchdog → строка уйдёт в failed с Retry. Вариант решения — держать keep-awake до
сообщения SW о конце сессии — требует UI-проверки (Chrome показывает значок звука на вкладке), поэтому
не сделано молча.
