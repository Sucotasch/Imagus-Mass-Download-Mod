...
Таймаут «нет работы дольше N — закрываем сессию» **сознательно не добавлен**: цена ложного
срабатывания — отмена медленного скана (галереи с lazy-load могут держать очереди пустыми долго,
а задачи, пришедшие после `scanInProgress=false`, помечаются `canceled`). Нужно измерение на живой
медленной галерее, а не догадка; вынесено в очередь отдельным пунктом.
