...
- **Verification scripts (all currently green):**
  - `node tools/md-unit-smoke.mjs` — MIME/ext helpers (`normalizeExt`, `getUrlExtension`, `isExcludedType`, `deriveFilename`) + **dedup contract**: SW `fileKey()` must equal content `_normalizeUrlKey()`, asserted for BOTH trees + the batch locks (D-1..D-8, ROWS-1, SUPERSEDE-1/2, RESUME-1, DIAG-1..3, HANDOFF-1/2: the `onUserScriptMessage` registration must be top-level/synchronous/unique and `mdClassifySendError` is EXECUTED). **Extraction gotcha:** it cuts functions out of source text assuming top-level declarations at column 0 — reformatting `service-core.js`/`content.js` breaks it.
