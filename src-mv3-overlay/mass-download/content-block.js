...
//   4. Message handlers (add inside PVI.onMessage, before closing `}`)
//   5. PVI methods (add at end of PVI object, before closing `};`)
//
// Each section is marked with >>> MASS-DOWNLOAD-START / <<< MASS-DOWNLOAD-END
...
// >>> MASS-DOWNLOAD-PROPERTIES-START
...
ambiguousUrlGroups: [],
// <<< MASS-DOWNLOAD-PROPERTIES-END


// ============================================================
// SECTION 3: Hotkey handler
// Location: Inside PVI.key_action, before the final `} else pv = false;`
// grep-pattern: The last `} else pv = false;` before `if (pv) pdsp(e);`
// ============================================================

// >>> MASS-DOWNLOAD-HOTKEY-START
} else if (key === cfg.keys.downloadAll) {
    if (e.shiftKey || e.ctrlKey) {
        PVI.downloadAll(doc);
        pv = true;
    } else pv = false;
// <<< MASS-DOWNLOAD-HOTKEY-END
} else pv = false;
if (pv) pdsp(e);
...
// Location: Inside PVI.onMessage, before closing `}`
// grep-pattern: `} else if (d.cmd === "download") {`
// Add AFTER the `download(d)` handler
// ============================================================

// >>> MASS-DOWNLOAD-MESSAGES-START
