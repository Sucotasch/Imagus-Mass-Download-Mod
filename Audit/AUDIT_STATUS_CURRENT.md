...
---
## 10. Дополнение 2026-09-12 — заход после ревью `REVIEW_BT_AND_V` (D-1, D-5, D-6, D-7, D-8, D-10)

Разделы 1–9 не переписаны (см. футер). Источник деталей: `Docs/FIX_PLAN_REVIEW_2026-09-12.md`
(раздел «Заход 2026-09-12 (вечер)»), сводка — `REPORT_GALLERY_BATCH_2026-09-06.md` §33.

| ID | Суть | Статус | Evidence |
|----|------|--------|----------|
| `D-1` (= `BT-02`) | Синхронный XHR в бандловом правиле E-Hentai: одна мёртвая страница галереи бросала из `res()` и обрывала цепочку | **Закрыто** иначе, чем планировалось: `hardenSieveRes()` патчит текст правила в `cacheSieve()` (дубль `_`-правила НЕ создаётся, видимый список правил не меняется, апдейт сива патч не отменяет) | `repro-d1-sieve-harden.mjs` 28/28 (A/B на настоящем тексте правила) |
| `D-5` | `Port.send` подставлял `Port.listener` колбэком → `Unchecked runtime.lastError` на каждую fire-and-forget команду | **Закрыто**: обёртка читает `runtime.lastError` и передаёт ответ дальше; канал `resolve` (`sendResponse`) сохранён | лок `md-unit-smoke` (D-5) + `postMessage: sendResponse` в том же лочке |
| `D-6` | Фильтр-`fetch` без `credentials` ⇒ cookie не отправлялись ⇒ session-gated хосты 403 на каждой валидации | **Закрыто** (HEAD, GET-fallback, валидация кандидатов); живая проверка на fetlife/e-hentai — за владельцем | локи `md-unit-smoke` (D-6) |
| `D-7` | Глобальный circuit breaker: шторм 403 на одном хосте подавлял валидацию для всех остальных | **Закрыто**: `breakerByHost` (per-host, метки времени, потолок 64, сброс по успеху); пейсинг/Avito — отклонён (нет замеров) | `repro-session-memory.mjs` 46/46 (хост B жив при breaker у A) |
| `D-8` | `scripts/verify-security.mjs` красный на каждом прогоне (ассертил Chrome-механику у FF-дерева) | **Закрыто**: проверяются `!importScripts(`, оба модуля в `manifest.background.scripts` и порядок. `scripts/` — локальный НЕотслеживаемый каталог, поэтому смоук-лок включается только при наличии файла | `node scripts/verify-security.mjs` → passed |
| `D-10` | Повторный скан той же страницы качал уже скачанное (`name (1).jpg`) | **ОТКАЧЕНО** (2026-09-12, после вопроса владельца): выгода была гипотезой из чтения кода, а цена — отказ в законном «скачать заново» (удалённые/перенесённые файлы); признак неверности — правка потребовала ВТОРОЙ правки против своего же вреда. Дедуп остаётся посканным, явный Retry — единственный санкционированный повтор, `Clear All` — чистый лист | надгробный лок в `tools/md-unit-smoke.mjs` (любое из имён машинерии валит смоук); `mutation-check` доказывает, что этот лок кусается |
| `D-11` | Держать keep-awake до конца сессии в SW | **Отклонено** (цена > польза; строка и так уходит в `failed` + Retry) | `Docs/FIX_PLAN_REVIEW_2026-09-12.md` |
| `D-3` / `D-4` | Авito: детектор лимитирования; fetlife: живой лог | **Открыто** — нужен Save Log с этих страниц, без данных это гадание | — |

### 10.1 Найдено живой проверкой владельца (2026-09-12, вечер)

| ID | Суть | Статус | Evidence |
|----|------|--------|----------|
| `ROWS-1` | Таблица прогресса выглядела как «в основном ошибки, 17 completed при 30 скачанных». Причина — порядок вытеснения строк: потолок `da.maxProgressRecords` сортировал от `completed: 0`, поэтому первыми удалялись строки, которые ПОДТВЕРЖДАЮТ факт скачивания, а ошибки оставались; плитки считались по этой же урезанной таблице | **Закрыто**: `mdEvictOldestRows()` (завершённая строка вытесняется раньше живой, внутри группы — самая старая); то же правило в вкладке; плитки переименованы (`Rows in List` / `Completed in List`); окно объясняется на странице и в Save Log | архив логов: расхождение счётчика и строк **только** на потолке (117/90, 77/44, 30/17), точное совпадение ниже потолка (34/34, 45/45, 33/33); `repro-row-window.mjs` 23/23 ×2 (A/B: 14 из 30 vs 30 из 30) |
| `ROWS-2` | Высокая доля 403 на `wimg.rule34.xxx` — и в фильтре, и в самой загрузке браузером | **Открыто, нужен живой замер** (частичные данные есть: прогон 20:29 — 20 из 40 completed это `sample_*.jpg`, и в КАЖДОМ из них полноразмерный `images/<hash>.png` был испробован и упал (403 на GET + 404 в браузере на угаданный `.jpg`), т.е. sample — лучшее из доступного; соотношение стабильно от прогона к прогону 20/20 — это ограничение на стороне хоста + угадывание расширения, а не ранний выход цепочки) (не регресс: тот же профиль в логах с 09-08; вероятное ограничение частоты). Без данных правка была бы гаданием. ⚠ **Поправка от `SUPERSEDE-1` (ниже):** «доля сбоев» считалась по СТРОКАМ, а строка = попытка, а не пункт; в прогоне 19-07-38 потеряно 2 пункта из 42, а 44 строки были мёртвыми кандидатами уже скачанных пунктов | в каждом прогоне ~половина строк-сбоев; `GET/403` (это статус НАШЕГО fetch'а) + вердикт браузера `SERVER_BAD_CONTENT` = HTTP 404; referer-ретрай проваливается единично |

### 10.2 Найдено живой проверкой владельца (2026-09-12, ночь): `SUPERSEDE-1`

| ID | Суть | Статус | Evidence |
|----|------|--------|----------|
| `SUPERSEDE-1` | Счётчик ошибок считал ПОПЫТКИ, а не пункты: каждый мёртвый кандидат фолбэк-цепочки навсегда оставался строкой `failed`. Прогон 19-07-38: 42 превью → 42 файла → 42 `COMPLETED`, но **46** `FAILED`, из которых 44 — URL, присутствующие в `attempts`-цепочке терминальной строки того же пункта (проверка по цепочке, а НЕ по basename: basename с другим расширением бывает другим файлом). Реально не скачано **2** пункта из 42 | **Закрыто**: `mdSupersedeAttempt()` в `advanceToNextCandidate`/`requeueNextCandidateForFilter` (мёртвый кандидат → `skipped` + `superseded`, строка и её URL/причина/кнопка Retry сохраняются; отменённые не трогаются); `mdItemFailedText()` во всех 9 терминальных точках («all N candidate URLs failed»); последняя попытка теперь попадает в `_attempts`; флаг `superseded` проброшен во вкладку (Retry для `failed\|canceled\|superseded`) и в Save Log | `repro-supersede-attempts.mjs` 42/42 ×2 дерева (A/B/C/D/E по числам того же лога); локи `SUPERSEDE` в `md-unit-smoke` + 5 мутаций в `mutation-check` (15/15) |
| `ROWS-3` | Строка-объяснение в Save Log («меньше строк, чем скачано») сравнивала ОБЩЕЕ число строк (`items.length`) с `downloaded + skipped` — на полном списке (100 из 100) условие всегда ложно. Живой прогон 19:46: `downloaded=41` рядом с `completed=40` без объяснения, и разница прочиталась владельцем как потерянный файл | **Закрыто**: чистая `mdLogCapNote(byStatus, stats)` сравнивает **completed-строки** с живым счётчиком и прямо говорит, что файлы на диске, а разницу дал потолок списка | `repro-row-window` 33/33 (19:46 → «1», 18:57 → «13», молчание при 34/34 и 0/0, регрессия «сравнение с items.length не возвращается»); сверка с диском: все 40 файлов на месте |
| `SUPERSEDE-2` | Текст причины врал: терминальная строка говорила `Server error: SERVER_BAD_CONTENT - trying alternate URL`, хотя (а) цепочка исчерпана и ничего не повторяется, (б) `SERVER_BAD_CONTENT` у Chromium — это **HTTP 404**, а не «ошибка сервера». Комментарий в коде «Chrome reports both 403 and 404 as SERVER_FORBIDDEN» — **неверен** | **Закрыто**: маппинг переписан по исходнику Chromium (`HandleSuccessfulServerResponse`, `components/download/internal/common/download_utils.cc`): 404 → `SERVER_BAD_CONTENT`, 403 → `SERVER_FORBIDDEN`, 401/407 → `SERVER_UNAUTHORIZED`, прочие 4xx/5xx → `SERVER_FAILED`; сырой enum остаётся в `attempts` лога | локи смоука на тексты 404/403/5xx + мутация «возврат старого маппинга» |

### 10.3 Найдено живой проверкой владельца (2026-09-13, ночь): `RESUME-1`

| ID | Суть | Статус | Evidence |
|----|------|--------|----------|
| `RESUME-1` | Скан навсегда встаёт на `Scan complete… Analyzing N complex items` после ПЕРЕЗАПУСКА сервис-воркера. Живой прогон 19:58: перезапуск на 36-й секунде, 8 файлов из 42, дальше в логе нет ни одного события. Механика: FIX-7 восстанавливает очереди и таблицу из снапшота, но не восстанавливает **незавершённый диалог со страницей** — страница ждёт ответный `groupAnalysisComplete` на запрос, который умер вместе со старым воркером; очереди пусты, поэтому D-9-проба (`mdProbeInitiatorTab`) видит ЖИВУЮ вкладку и ждёт вечно (D-9 закрывал только закрытую) | **Закрыто** (оба дерева): восстановление шлёт странице `{cmd:'resumeGroupAnalysis'}` (страница держит группы в памяти и переотправляет их — безопасно, т.к. дедуп-ключи восстановлены, а терминальные строки снапшота сеют их сразу, без ожидания дебаунса); ответное окно `MD_RESUME_ANSWER_MS = 20000`; его закрывают приход групп, `done` ИЛИ новое подтверждение `resumeGroupAnalysisAck` («жив, ещё иду по DOM» — без него живая занятая страница читалась бы как мёртвая); молчание дольше окна ⇒ `mdConcludeAbandonedScan` (сессия доигрывается, строки сохраняют Retry) | `repro-resume-after-restart.mjs` 50/50 ×2 дерева (в т.ч. E2: страница, подтвердившая запрос, не завершается и через 2 минуты); `repro-session-snapshot` 49/49 (`T14`: восстановление реально шлёт запрос и не шлёт при уже полученном `done`); локи `RESUME` + 7 мутаций → `mutation-check` 23/23 |
| `RESUME-2` (= остаток D-9) | Замороженная (`frozen`/`discarded`) вкладка, которая проснётся ПОСЛЕ завершения сессии, остаётся со своим статусом и `downloadAllActive=true` (новый скан в этой вкладке откажет до перезагрузки страницы). Файлы при этом доезжают (дедуп на месте) | **Осознанно оставлено** (цена заморозки — самого пользователя, страница прямо предупреждает не покидать её); принудительно закрывать скан НЕЛЬЗЯ: `stopScanning`/`groupAnalysisComplete` отменили бы DOM-обход живой вкладки и потеряли бы её оставшиеся элементы | `Docs/FIX_PLAN_REVIEW_2026-09-12.md` (RESUME-1) |

**Поправка к §9.4:** «платформенная стена» Chrome+pixiv относится к `chrome.downloads`, а в коде
ЕСТЬ обход — offscreen-тир (`mdOffscreenSupported()` в `service-core.js` + гейт `mdDnrRuleActiveFor`,
подключён и в фильтр-фазе, и в download-пути). Работает ли он живьём — не проверено; это вопрос
живого прогона, а не кода.

---
*Создан 2026-08-23 консолидацией статусов четырёх аудитов. Новые аудиты дописывать сюда (новая секция), историю не переписывать. Дополнение §9 — 2026-09-11. Дополнение §10 — 2026-09-12 (вечер: `ROWS-1`; ночь: `SUPERSEDE-1`, `SUPERSEDE-2`; живой прогон 19:46: `ROWS-3`; живой прогон 19:58 — 2026-09-13: `RESUME-1`, `RESUME-2`).*
