...
| `ROWS-2` | Высокая доля 403 на `wimg.rule34.xxx` — и в фильтре, и в самой загрузке браузером | **Открыто, нужен живой замер** (не регресс: тот же профиль в логах с 09-08; вероятное ограничение частоты). Без данных правка была бы гаданием. ⚠ **Поправка от `SUPERSEDE-1` (ниже):** «доля сбоев» считалась по СТРОКАМ, а строка = попытка, а не пункт; в прогоне 19-07-38 потеряно 2 пункта из 42, а 44 строки были мёртвыми кандидатами уже скачанных пунктов | в каждом прогоне ~половина строк-сбоев; `GET/403` (это статус НАШЕГО fetch'а) + вердикт браузера `SERVER_BAD_CONTENT` = HTTP 404; referer-ретрай проваливается единично |
