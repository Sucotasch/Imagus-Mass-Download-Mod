...
| `D-10` | Повторный скан той же страницы качал уже скачанное (`name (1).jpg`) | **Закрыто**: память сессии браузера `mdSessionDownloads` (дебаунс 2 с, FIFO 4000) + чистая `mdDedupSkipReason()`; строка видима, `completed` не понижается до `skipped`, Retry обходит дедуп | `repro-session-memory.mjs` (гонка Clear, потолок, приоритеты) |
