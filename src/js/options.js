			...
			while (args.idx--) {
				...
				while (attrs.idx--) {
					...
					attrnode.setAttribute(
						attrs[attrs.idx],
						els[l].getAttribute(args[args.idx][0] + '-' + attrs[attrs.idx])
					);
				}
			}
...
var load = function() {
	var
		fields = document.querySelectorAll('input[name*=_], select[name*=_], textarea[name*=_]'),
		i = fields.length,
		j, m, fld, fld_type, shosts, pref, prefs = {};
		...
		if (pref[0] === 'tls' && pref[1] === 'sendToHosts') {
			...
			if (m && m.length) {
				for (j = 0; j < m.length; ++j) {
					shosts.push(m[j].op === ';' ? ';' + m[j].txt : m[j].op +
						(m[j].rules || m[j].opts || '') + ':' + m[j].url);
				}
			}
			...
			if (prefs.keys[m] !== void 0) {
				fld.value = fld.defValue = prefs.keys[m].toUpperCase();
			}
		} else {
			if (prefs[pref[0]] && prefs[pref[0]][pref[1]] !== void 0) {
				...
				if (fld_type === 'checkbox') {
					...
					} else if (fld_type === 'text' && fld.previousElementSibling
						&& fld.previousElementSibling.getAttribute('type') === 'color') {
...
var save = function() {
	...
	for (i = 0; i < fields.length; ++i) {
		...
		if (pref[0] === 'tls' && pref[1] === 'sendToHosts') {
			...
			} else if (fldType === 'range' || fldType === 'number'
				|| fld.classList.contains('number')) {
				let value = parseFloat(fld.value);
				if (isNaN(value)) {
					value = parseFloat(fld.defaultValue);
				}
				if (fld.hasAttribute('min')) {
					value = Math.max(parseFloat(fld.min), value);
				}
				if (fld.hasAttribute('max')) {
					value = Math.min(parseFloat(fld.max), value);
				}
				prefs[pref[0]][pref[1]] = value;
				fld.value = value;
			} else {
...
var prefs = function(data, options, ev) {
	var i, pref_keys = ['hz', 'keys', 'tls', 'grants'];
		...
		if (hash === 'sieve') {
			...
			section.querySelector('.action_buttons').onclick = function(e) {
				switch (e.target.textContent) {
					case '↓': ImprtHandler(_('SC_PREFS'), prefs, {'overwrite': 1}); break;
					case '↑': prefs(0, 0, e); break;
				}
			};

			if (args[0]) {
				$(args[0] === '0' ? 'app_installed' : 'app_updated').style.display = 'block';
			}
			...
			Port.listen(function(response) {
				...
				for (alpha2 in locales_json) {
					...
					locales.push({tag: 'tr', nodes: [
						{tag: 'td', attrs: locales_json[alpha2]['%'] ? {title: locales_json[alpha2]['%'] + '%'} : null, text: alpha2 + ', ' + locales_json[alpha2].name},
						td2
					]});
...
window.addEventListener('load', function() {
	...
	document.forms[0].addEventListener('contextmenu', function(e) {
		e.stopPropagation();
		var t = e.target;
	...
	reset_button.addEventListener('click', function(e) {
		if (reset_button.pending) {
			...
			setTimeout(function() {
				...
				[].forEach.call(
					document.querySelectorAll('output + input[type="range"]') || [],
					fill_output
				);
			}, 0xf);
	...
	Port.send({'cmd': 'cfg_get', 'keys': ['hz', 'keys', 'tls', 'grants', 'da']});
}, false);
