...
if (imgDoc && imgDoc.parentNode === doc.body && imgDoc.src === win.location.href) {
	return;
}
...
var shortcut = {
	specKeys: {
		...
		16: 'shift', 17: 'ctrl', 18: 'alt', // control keys are not capital
		27: 'Esc', 32: 'Space',
...
var onMouseDown = function(e) {
	...
	if (e.button === 0) {
		...
		if (platform.opera
			&& (e.target.clientWidth && e.offsetX >= e.target.clientWidth
				|| e.target.clientHeight && e.offsetY >= e.target.clientHeight)) {
			return;
		}
	...
	if (platform.chrome && (e.target.href || (d = e.target.parentNode) && d.href)) {
		e.preventDefault();
	}
};
...
var onContextMenu = function(e) {
	...
	if (i) {
		...
		pdsp(e, false);
	} else if (e.ctrlKey && !elapsed && !e.shiftKey && !e.altKey
			&& cfg.tls.opzoom && PVI.state < 2
			&& (i = (checkIMG(e.target)
...
var PVI = {
	...
	*/
	...
	convertSieveRegexes: function() {
		...
		if (!Array.isArray(s) || !(i = s.length) || typeof (s[0].link || s[0].img) !== 'string') {
			return;
		}

		while (i--) {
			if (s[i].link) {
				s[i].link = RegExp(s[i].link, s[i].ci && (s[i].ci & 1) ? 'i' : '');
			}

			if (s[i].img) {
				s[i].img = RegExp(s[i].img, s[i].ci && (s[i].ci & 2) ? 'i' : '');
			}
		}
	},
	create: function() {
		...
		PVI.DIV.IMGS_ = PVI.DIV.IMGS_c = PVI.LDR.IMGS_ = PVI.LDR.IMGS_c =
			PVI.VID.IMGS_ = PVI.VID.IMGS_c = PVI.IMG.IMGS_ = PVI.IMG.IMGS_c = true;
		...
		PVI.DIV.style.cssText = 'margin: 0; padding: 0; ' + (cfg.hz.css || '') + '; visibility: visible; cursor: default; display: none; z-index: 2147483647; ' +
		...
		if (platform.chrome || platform.maxthon) {
			PVI.VID.poster = 'data:image/gif;base64,R0lGODlhAQABAAAAACwAAAAAAQABAAA=';
		}
		...
		if (cfg.hz.hideIdleCursor >= 50) {
			...
			PVI.DIV.addEventListener('mousemove', function(e) {
				if (e.target !== PVI.CNT
					|| PVI.CNT === PVI.VID && PVI.VID.clientHeight - 35 < (e.offsetY || e.layerY || 0)) {
		...
		if (x[y + 'Property']) {
			...
			['opacity', 'left', 'top', 'width', 'height'].forEach(function(el) {
				var idx = p[0].indexOf(el), val = parseFloat(p[1][idx]) * 1000;
	...
	createCAP: function() {
		...
		buildNodes(PVI.CAP, [
			{tag: 'b', attrs: {style: 'display: none; ' + platform['transition_css'] + ': background-color .1s; border-radius: 3px; padding: 0 2px'}},
		...
		PVI.CAP.style.cssText = 'left:0; right:auto; display:block; cursor:default; position:absolute; width:auto; height:auto; border:0; white-space: '
		 + (cfg.hz.capWrapByDef ? 'pre-line' : 'nowrap') + '; font:13px/1.4em "Trebuchet MS",sans-serif; background:rgba(' + (n ? '255,255,255,.95' : '0,0,0,.75') + ') !important; color:#' + (n ? '000' : 'fff') + ' !important; box-shadow: 0 0 1px #' + (n ? '666' : 'ddd') + ' inset; padding:0 4px; border-radius: 3px';
	...
	flick_caption: function() {
		...
		s.backgroundColor = s.backgroundColor === PVI.palette.pile_bg
			? 'red'
			: PVI.palette.pile_bg;
	},
	updateCaption: function() {
		...
		if (PVI.TRG.IMGS_album) {
			if (c.firstChild.style.display === 'none'
				&& (h = PVI.stack[PVI.TRG.IMGS_album]) && h[2]) {
		...
		if (PVI.CNT !== PVI.IFR) {
			...
			if (cfg.hz.capWH || c.state === 2) {
				...
				h.textContent = (PVI.TRG.IMGS_SVG ?
	...
	onAttrChange: function(e) {
		...
		switch (e.attrName) {
			case 'style':
				...
				if ( (!bgImg || e.prevValue.indexOf(bgImg.slice(5, -2)) !== -1)
					&& e.prevValue.indexOf('opacity') === -1
					&& target.style.cssText.indexOf('opacity') === -1 ) {
					return;
				}
				/* falls through */
			case 'href': case 'src': case 'title': case 'alt':
				if (target === PVI.TRG) {
	...
	getImages: function(el) {
		...
		if ( isHTMLElement ) {
			if ( el.childElementCount > 0 && el.childElementCount < 3 ) {
				...
				if ( imgs.src && !/\S/.test(el.textContent)
		...
		if ( imgs.imgSRC_o ) {
			if ( !isHTMLElement ) {
				imgs.imgSRC_o = PVI.normalizeURL(imgs.imgSRC_o);
			// Ignore "spacer.gif"
			} else if ( el.naturalWidth > 0 && el.naturalWidth < 3
		...
		if ( el.style.backgroundImage[0] === 'u' ) {
			...
			if ( p.offsetParent === el.offsetParent
				&& p.style && p.style.backgroundImage[0] === 'u' ) {
				if ( Math.abs(p.offsetLeft - el.offsetLeft) <= 10
	...
	_replace: function(rule, addr, http, param, to, trg) {
		...
		if (rule.dc && (param === 'link' && rule.dc !== 2
			|| param === 'img' && rule.dc > 1)) {
	...
	resolve: function(URL, rule, trg, nowait) {
		...
		if ( trg.IMGS_c_resolved && typeof trg.IMGS_c_resolved.URL !== 'string' ) {
			return false;
		}
		...
		if (!cfg.hz.waitHide
			&& (PVI.fireHide && PVI.state > 2
				|| PVI.state === 2 || PVI.hideTime && Date.now() - PVI.hideTime < 200)) {
			nowait = true;
		}
	...
	find: function(trg, x, y) {
		...
		do {
			...
			if ( n instanceof win.HTMLElement ) {
				...
				if ( platform.opera ) {
					if (trg.childElementCount && ((tmp_el = doc.evaluate('.//*[self::img[@src] or self::*[contains(@style, "background-image:")]] | preceding-sibling::img[@src][1] | following-sibling::img[@src][1]', trg, null, 8, null).singleNodeValue)
							&& (tmp_el.src !== void 0 || !/\S/.test(trg.textContent))
						|| n.parentNode.style.backgroundImage[0] === 'u'
							&& (tmp_el = n.parentNode) && tmp_el.childElementCount < 3
						|| n.style.backgroundImage[0] === 'u' && (tmp_el = n))
							&& Math.abs(trg.offsetWidth - tmp_el.offsetWidth) <= 25
							&& Math.abs(trg.offsetHeight - tmp_el.offsetHeight) <= 25) {
					...
					for ( i = 0; i < tmp_el.length && i < 5; ++i ) {
						...
						if ( !tmp_el[i].currentSrc
							&& tmp_el[i].style.backgroundImage.lastIndexOf('url(', 0) !== 0 ) {
							continue;
						}
						...
						if ( x >= elRect.left && x < elRect.right
							...
							if ( trgRect.left - 10 <= elRect.left
			...
			for ( i = 0; rule = cfg.sieve[i]; ++i ) {
				...
				if (rule.res && (!tmp_el || (!rule.to && rule.url))) {
					if ( win.location.href.replace(rgxHash, '')
					...
					ret = rule.url
						? URL.replace(rule[tmp_el ? 'img' : 'link'], rule.url)
						: URL;
					...
					ret = PVI.replace(rule, URL, n.href, tmp_el ? 'img' : 'link', trg);
				}
				...
				if (typeof ret === 'string' && n !== trg && trg.hasAttribute('src')
		...
		if (!ret && ret !== null) {
			...
			if (imgs && (imgs.imgSRC || imgs.imgBG)) {
				...
				for (; rule = cfg.sieve[i]; ++i) {
					if (use_img[0] ||
						(rule.img && (imgs.imgSRC && rule.img.test(imgs.imgSRC)
							|| imgs.imgBG && (use_img[1] = rule.img.test(imgs.imgBG))))) {
		...
		if (rule && rule.loop && typeof ret === 'string'
			&& rule.loop & (use_img ? 2 : 1) ) {
			// protection from infinite loop
			if (trg.nodeType !== 1 && ret === trg.href || trg.IMGS_loop_count > 5) {
				return false;
			}
		...
		if (ret && (typeof ret === 'string' || Array.isArray(ret))) {
			...
			URL = [
				...
				trg.nodeType === 1 && trg.src && trg.hasAttribute('src')
		...
		if (trg.localName === 'img' && trg.hasAttribute('src')) {
			...
			for ( i = 0; i < tmp_el.length; ++i ) {
				...
				while ( j-- ) {
					...
					if ( descriptor === 'x' ) {
						srcItem[1] = trg.naturalWidth * srcItem[1].slice(0, -1);
			...
			do {
				...
				switch ( tmp_el.display ) {
					...
					case 'list-item': case 'table-caption':
						break;
					default:
						continue;
				}
		...
		if (trg.IMGS_caption) {
			if (!cfg.hz.capLinkText && trg.IMGS_caption === tmp_el
	...
	show: function(msg, delayed) {
		...
		if (!delayed && typeof msg === 'string') {
			...
			PVI.HD_cursor(true);
			PVI.BOX = PVI.LDR;
			PVI.LDR.style.backgroundColor = cfg.hz.LDRbgOpacity < 100
				? PVI.palette[msg].replace(/\(([^\)]+)/, 'a($1, ' + (cfg.hz.LDRbgOpacity / 100))
				: PVI.palette[msg];
		...
		if (msg) {
			...
			if ((PVI.state < 3 || PVI.BOX === PVI.LDR) && box.display === 'none'
			...
			if (box.opacity === '0' && (PVI.BOX === PVI.DIV && PVI.anim.opacity
		...
		if (msg === false) {
			...
			var
				fs = cfg.hz.fullspace || cfg.hz.placement === 2,
				cap_size = PVI.CAP && PVI.CAP.overhead && !(PVI.DIV.curdeg % 360) && PVI.CAP.state !== 0
			...
			switch (cfg.hz.placement) {
				case 1: // cursor at side of the image
					...
					if (hH && cfg.hz.fullspace && (winH - vH <= winW - vW || vW <= (x < rSide ? rSide : x))) {
						hH = false;
					}
				...
				case 3: // no cover
					...
					hH = (x < rSide ? rSide : x) < vW
						...
						|| (y < bSide ? bSide : y) >= vH && winW >= vW
							...
							&& (PVI.TBOX.width >= winW / 2 || (Math.abs(PVI.x - left) >= winW / 3.5));
					...
					if (!cfg.hz.fullspace || (hH ? vH <= (y < bSide ? bSide : y) : vW <= (x < rSide ? rSide : x))) {
						...
						if (hH) {
							...
							if (fs > 10) {
								left = x < rSide ? Math.max(left, PVI.TBOX.left) : Math.min(left, PVI.TBOX.right - vW);
							}
						} else {
							top = (PVI.TBOX.top + PVI.TBOX.bottom - vH) / 2;

							if (fs < 0.1) {
								top = y < bSide ? Math.min(top, PVI.TBOX.top) : Math.min(top, PVI.TBOX.bottom - vH);
							}
						}
					}
					break;
	...
	set: function(src) {
		...
		if ( /^[^?#]+\.(?:m(?:4[abprv]|p[34])|og[agv]|webm)(?:$|[?#])/.test(src)
	...
	history: function(manual) {
		...
		if (!PVI.CNT || !PVI.TRG
			|| platform.crx && chrome.extension && chrome.extension.inIncognitoContext
			|| platform.maxthon) {
			return;
		}
		...
		if (PVI.TRG.IMGS_album) {
			...
			if (!manual && (url.in_history || url.length > 4 && url[0] === 1)) {
				return;
			}
	...
	HD_cursor: function(reset) {
		if (!PVI.TRG || !reset && (cfg.hz.capWH || PVI.TRG.IMGS_HD === void 0)) {
			return;
		}
	...
	isEnlargeable: function(img, oImg, isOverflow) {
		...
		return w < topWinW * 0.9 && 100 - w * 100 / ow >= cfg.hz.zoomresized
			|| h < topWinH * 0.9 && 100 - h * 100 / oh >= cfg.hz.zoomresized;
	},
	...
	assign_src: function() {
		...
		if (PVI.TRG.IMGS_album) {
			...
			if (PVI.TRG !== PVI.HLP && PVI.TRG.IMGS_thumb
			...
			if (PVI.CNT === PVI.IMG && !PVI.IMG.loaded && cfg.hz.thumbAsBG
				&& PVI.TRG.IMGS_thumb !== false && !PVI.TRG.IMGS_album) {
				var inner_thumb, w, h;
				...
				if (PVI.TRG.IMGS_thumb === PVI.IMG.src) {
					...
					if (!PVI.TRG.IMGS_thumb_ok) {
						...
						PVI.TRG.IMGS_thumb_ok = Math.abs(
								PVI.IMG.naturalWidth / PVI.IMG.naturalHeight - w / h
							) <= 0.2;

						w = w < 1024 && h < 1024 &&
							w < PVI.IMG.naturalWidth && h < PVI.IMG.naturalHeight;
					}
					...
					if (w && PVI.TRG.IMGS_thumb_ok) {
						if (PVI.interlacer) {
							...
							if (cfg.hz.thumbAsBGOpacity > 0) {
								...
								h.appendChild(doc.createElement('div')).style.cssText = 'width: 100%; height: 100%; background-color: rgba('
									+ (w >> 16) + ',' + (w >> 8 & 0xff) + ',' + (w & 0xff) + ',' + parseFloat(cfg.hz.thumbAsBGOpacity) + ')';
							}
	...
	hide: function(e) {
		...
		if (animDIV && PVI.anim.left || animLDR) {
			PVI.BOX.style.left = (cfg.hz.follow
				? e.clientX || PVI.x
				: parseInt(PVI.BOX.style.left, 10) + PVI.BOX.offsetWidth / 2
			) + 'px';
		}

		if (animDIV && PVI.anim.top || animLDR) {
			PVI.BOX.style.top = (cfg.hz.follow
				? e.clientY || PVI.y
				: parseInt(PVI.BOX.style.top, 10) + PVI.BOX.offsetHeight / 2
			) + 'px';
		}
	...
	reset: function(preventImmediateHover) {
		...
		if (PVI.CAP) {
			PVI.CAP.firstChild.style.display = PVI.CAP.style.display = 'none';
		}
	...
	key_action: function(e) {
		...
		if (shortcut.isModifier(e)) {
			...
			if (e.repeat || shortcut.key(e) !== cfg.hz.actTrigger) {
				return;
			}
		...
		if (cfg.hz.scOffInInput && pv && /*pv !== PVI.INP && */(pv.isContentEditable
				|| (pv = pv.nodeName.toUpperCase()) && (pv[2] === 'X' // teXtarea
					|| pv === 'INPUT'))) {
			return;
		}
		...
		if (e.altKey && e.shiftKey) {
			...
			if (e.ctrlKey) {
				...
				if (PVI.CNT === PVI.VID
					&& (win.fullScreen || doc.fullscreenElement
				...
				if (PVI.CNT === PVI.VID) {
					...
					if (key === 'Space') {
						if (e.shiftKey) {
							if (!PVI.VID.audio) {
								PVI.VID.controls = PVI.VID._controls = !PVI.VID._controls;
							}
						} else {
							if (PVI.VID.paused) {
								PVI.VID.play();
						...
						if (PVI.VID.audio) {
							PVI.VID.currentTime += key === 'PgDn' ? 4 : -4;
						} else {
							PVI.VID.pause();
							PVI.VID.currentTime = (PVI.VID.currentTime * 25 + (key === 'PgDn' ? 1 : -1)) / 25 + 0.00001;
						}
					} else {
						pv = null;
					}
				...
				if (!pv && PVI.TRG.IMGS_album) {
					switch (key) {
						case 'End':
							if (e.shiftKey
								&& (pv = prompt('#', PVI.stack[PVI.TRG.IMGS_album].search || '') || null)) {
						...
						break;
						default:
							pv = (key === 'Space' && !e.shiftKey || key === 'Right' || key === 'PgDn' ? 1 : -1) *
									(e.shiftKey && key !== 'Space' ? 5 : 1);
					}
			...
			} else if (key === cfg.keys.mOrig || key === cfg.keys.mFit
				|| key === cfg.keys.mFitW || key === cfg.keys.mFitH) {
				PVI.resize(key);
				...
				if (e.shiftKey) {
					...
					PVI.CAP.style.whiteSpace = PVI.CAP.style.whiteSpace === 'nowrap' ? 'normal' : 'nowrap';
				}
	...
	fzClickAct: function(e) {
		...
		if (e.target === PVI.CAP
			...
			if ((e.offsetY || e.layerY || 0) < Math.min(PVI.CNT.clientHeight - 40, 2 * PVI.CNT.clientHeight / 3)) {
				PVI.reset(true);
			} else if ((e.offsetY || e.layerY || 0) < PVI.CNT.clientHeight - 40
	...
	wheeler: function(e) {
		...
		if (PVI.TRG && PVI.TRG.IMGS_album && cfg.hz.pileWheel
			// Scroll in the top left corner
			&& (!PVI.fullZm || e.clientX < 50 && e.clientY < 50
				// or over the caption
	...
	m_over: function(e) {
		...
		if ( cfg.hz.deactivate && (PVI.freeze || e[cfg._freezeTriggerEventKey]) ) {
			return;
		}
		...
		if (cache || src || (src = PVI.find(trg, e.clientX, e.clientY)) || src === null) {
			...
			if (!isFrozen && (!cfg.hz.waitHide || cfg.hz.delay < 15)
				&& (PVI.fireHide && PVI.state > 2
					|| (PVI.state === 2 || PVI.hideTime && Date.now() - PVI.hideTime < 200))) {
				if (PVI.hideTime) {
					PVI.hideTime = 0;
	...
	load: function(src) {
		if ((cfg.hz.waitHide || !cfg.hz.deactivate) && PVI.anim.maxDelay && !PVI.iFrame) {
			win.addEventListener('mousemove', PVI.m_move, true);
		}
		...
		if (src === void 0) {
			src = cfg.hz.delayOnIdle && PVI.TRG.IMGS_c_resolved || PVI.SRC;
		}
		...
		if (src === null || (src && src.params) || src === false) {
			if (src === false
				|| src && (src = PVI.resolve(src.URL, src.params, PVI.TRG)) === 1) {
				PVI.create();
				PVI.show('R_js');
				return;
			}
	...
	m_move: function(e) {
		...
		if (PVI.fullZm) {
			...
			if (PVI.fullZm > 1 && e[0] !== true) {
				...
				x = (w - PVI.DBOX['wpb'] > winW
						? -(PVI.x * (w - winW + 80) / winW) + 40
						: (winW - w) / 2)
					+ rot - PVI.DBOX['ml'];
				y = (h - PVI.DBOX['hpb'] > winH
		...
		if ( PVI.freeze && !cfg.hz.deactivate && !e[cfg._freezeTriggerEventKey] ) {
			return;
		}
		...
		} else if (e.target.IMGS_ && PVI.TBOX
			&& (PVI.TBOX.Left > e.pageX || PVI.TBOX.Right < e.pageX
				|| PVI.TBOX.Top > e.pageY || PVI.TBOX.Bottom < e.pageY)
		...
		} else if (cfg.hz.move && PVI.state > 2 && !PVI.timers.m_move
			&& (PVI.state === 3 || cfg.hz.placement < 2 || cfg.hz.placement > 3)) {
	...
	preload: function(e) {
		...
		if (!nodes || nodes.IMGS_ || nodes.nodeType !== 1
		...
		nodes = function() {
			...
			while (node = PVI.preloading.shift()) {
				if ((node.nodeName.toUpperCase() === 'A' && node.childElementCount)
					|| node.IMGS_c_resolved || node.IMGS_c
					|| typeof node.IMGS_caption === 'string' || node.IMGS_thumb) {
					continue;
				}
				...
				if (src = PVI.find(node)) {
					...
					if (Array.isArray(src)) {
						...
						img.onerror();
					} else if (typeof src === 'string' && !rgxIsSVG.test(src)){
						(new Image).src = src;
					}
	...
	winOnMessage: function(e) {
		...
		if (cmd === 'toggle' || cmd === 'preload' || cmd === 'isFrame') {
			...
			while ( i-- ) {
				...
				try {
					if ( frms[i].location.href.lastIndexOf('about:', 0) === 0 ) {
						continue;
					}
				} catch (ex ) {}
	...
	onMessage: function(d) {
		...
		if ( d.cmd === 'resolved' ) {
				...
				if (d.m.length) {
					if (Array.isArray(d.m[0])) {
						...
						d.m.forEach(function(el) {
							if (Array.isArray(el[0]) && el[0].length === 1) {
								el[0] = el[0][0];
							}
						});
			...
			if (d.m) {
				if (!d.noloop && !trg.IMGS_album && typeof d.m === 'string'
					&& (d.loop || rule.loop
						&& rule.loop & (d.params.rule.loop_param === 'img' ? 2 : 1)) ) {
			...
			PVI.downloadAll(document, arguments[2]);
			return true;
		} else if (d.cmd === 'hello') {
			var e = !!PVI.DIV;
			PVI.init(null, true);
	...
	init: function(e, deinit) {
		if (deinit) {
			...
			if (PVI.DIV) {
				...
				PVI.BOX = PVI.DIV = PVI.CNT = PVI.VID = PVI.IMG
					= PVI.CAP = PVI.TRG = PVI.interlacer = null;
			}
			...
			if (e !== void 0) {
				...
				if (cfg && !cfg.hz.deactivate && cfg.hz.actTrigger === '0') {
					cfg = null;
				}
		...
		doc[e](platform.wheel, PVI.scroller, {capture: true, passive: true});
...
PVI.processNextInQueue();
	...
	processNextInQueue: function() {
		...
		const onResolved = (result) => {
            ...
            // Re-check the flag before scheduling the next call
            if (!PVI.downloadAllActive) {
                PVI._stopKeepAwake('Scanning canceled.');
                return;
            }
			...
			// If no result or duplicate, process next item immediately
...
PVI.processNextInQueue();
		...
		if (src === false) { // PVI.find explicitly failed
			onResolved(null);
		} else { // It returned a URL or null (for async), so we call load
			PVI.load(src);
			timeout = setTimeout(() => onResolved(null), ((cfg.da && cfg.da.resolutionTimeout) || 8) * 1000); // Use timeout from settings
		}
	}
};
